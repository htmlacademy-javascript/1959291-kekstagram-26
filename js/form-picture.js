...
const addEffectPreviewFilter = (filter, filterValue) => {
  if (filter === 'phobos') {
    imgUploadPreviewElement.style.filter = `${filterEffectTypes[filter]}(${filterValue}px)`;
    // записываем значение слайдера в поле ввода
    effectLevelValueElement.value = `${filterValue}px`;
    return;
  }
  if (filter === 'marvin') {
    imgUploadPreviewElement.style.filter = `${filterEffectTypes[filter]}(${filterValue}%)`;
...
const addSlider = (data) => {
  if (data.value === 'none') {
    ...
    effectLevelSliderElement.noUiSlider.updateOptions(sliderEffectParameters[`${data.value}`]);
